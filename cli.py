...
def cmd_investigate(a) -> int:
    """Run the five agents against one breach, without waiting for the clock."""
    from signal_service import evaluate as ev, store
    ...
    if target.startswith("BRC"):
        ...
        # a kpi name: evaluate it now and investigate whatever it says
        kpi = get(target)
        ...
        if not verdict.breached:
            print(f"  {target} is not breached ({verdict.value}{kpi.unit}, "
                  f"normally {verdict.baseline:g}). Investigating anyway.")
        store.setup()
        store.record(breach)
